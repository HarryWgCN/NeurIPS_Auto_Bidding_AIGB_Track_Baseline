...
from bidding_train_env.strategy import DdCPALossStrategy
...
def run_test():
    ...
    overall_reward = 0
    overall_score = 0
    for data_id in range(20, 25):
        data_loader = TestDataLoader(file_path=f'/home/disk2/auto-bidding/data/traffic_new_final/period-{data_id}.csv')
        score_all = 0
        reward_all = 0
        cost_all = 0
     ...
     # ----------------------------------------- stimulate agent init ↓----------------------------------------
        ...
        random_sample = df.sample(n=stimulate_agent_num*3+1)
        print(random_sample)
        ...
        # 传入模型编号
        agent = PlayerBiddingStrategy(budget=budget_cpa_sampled[-1][0], cpa=budget_cpa_sampled[-1][1])
        # agent = DdCPALossStrategy(budget=budget_cpa_sampled[-1][0], cpa=budget_cpa_sampled[-1][1])
        # agent = CPABiddingStrategy(budget=budget_cpa_sampled[-1][0], cpa=budget_cpa_sampled[-1][1])
        # agent = base_dd_bidding_strategy(budget=budget_cpa_sampled[-1][0], cpa=budget_cpa_sampled[-1][1])
        # agent = DtBiddingStrategy(budget=budget_cpa_sampled[-1][0], cpa=budget_cpa_sampled[-1][1])
        print(agent.name)
    ...
    #------------------------------------- stimulate agent init ↑----------------------------------------------
        ...
        overall_cost += cost_all / len(keys)
        overall_reward += reward_all / len(keys)
        overall_score += score_all / len(keys)
    logger.info(f'Score: {overall_score}')
    logger.info(f'Reward: {overall_reward}')
    logger.info(f'Cost: {overall_cost}')